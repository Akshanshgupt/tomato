...
const fetchRestaurants = async()=>{
  ...
  try {
    setLoading(true)
    const {data} = await axios.get(`${restaurantService}/api/restaurant/all`,{params:{
  ...
  return (
    <div className='mx-auto max-w-7xl x-4 py-6'>
{
   restaurants.length>0 ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">

    {
      restaurants.map((res)=>{
        ...
        const distance = getDistanceKm(location.latitude,
           location.longitude,
           resLat,
           resLng
        )
        return  <RestaurantCart key ={res._id} id ={res._id}
