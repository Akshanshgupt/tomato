    ...
    <div className='mx-auto max-w-5xl px-4 py-6 space-y-6'>
      ...
      <div className="space-y-4">
         {cart.map((cartItem:ICart)=>{
            ...
            <img src={item.image} alt="" className='h-20 w-30 rounded object-cover' />
            ...
            <div className="flex items-center gap-3">
              <button className='rounded-full border p-2 hover:bg-gray-100 disabled:opacity-50' disabled={isLoading} onClick={()=>decreaseQty(item._id)}>
                {isLoading ?<VscLoading size ={16} className='animate-spin' />:<BiMinus size={16} />}</button>
                ...
                {isLoading ?<VscLoading size ={16} className='animate-spin' />:<BiPlus size={16} />}</button>
            </div>
       ...
       <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
        ...
        <div className="flex justify-between text-base font-semibold border-t pt-2">
          ...
          <button onClick={checkout} className={`mt-3 w-full rounded-lg bg-[#E23744] py-3 text-sm font-semibold text-white hover:bg-red-800 ${!restaurant.isOpen?"opacity-50 cursor-not-allowed":""} `}
           disabled={!restaurant.isOpen}
          >
           {!restaurant.isOpen?"Restaurant is Closed":"Proceed to Checkout"} 
          </button>

          <button onClick={clearCart} className="mt-3 w-full rounded-lg bg-[#232222] py-3 text-sm font-semibold text-white hover:bg-gray-900 flex justify-center items-center gap-3"
