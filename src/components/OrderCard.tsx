        ...
        <span className= {`rounded-full px-3 py-1 text-xs font-medium ${statusColor(order.status)}`}>
         ...
         <div className="text-sm text-gray-600 space-y-1">
           ...
           {order.status==="ready_for_rider" && retryVisible && <div className='pt-2'>
            <button className='w-full rounded-lg border-[#e23744] py-2 text-xss font-semibold text-[#e23744] hover:bg-red-50 disabled:opacity-50 ' onClick={()=>updateStatus("ready_for_rider")}>Retry Ready for Rider</button>
           </div>
           } 
